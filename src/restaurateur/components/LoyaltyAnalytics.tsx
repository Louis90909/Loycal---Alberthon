...
const LoyaltyAnalytics: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            ...
            try {
                ...
                const data = await backend.getAnalytics(
                    user.restaurantId
                );
    ...
    return (
        <div className="space-y-8 animate-fade-in">
                ...
                <div className="flex bg-white rounded-xl p-1.5 border border-gray-200 shadow-sm">
                    {(['7j', '30j', '90j'] as const).map(r => (
                        <button
                            ...
                            className={`px-5 py-2 rounded-lg text-sm font-semibold transition-all ${timeRange === r
            ...
            {
                (() => {
                    const baseRev = (stats?.totalRevenue || 5000) / 7;
                    const dynamicRevenueForecast = MOCK_REVENUE_FORECAST.map((d, i) => {
                        const factor = [0.8, 0.9, 0.85, 1.1, 1.4, 1.6, 1.1][i];
                        return {
                            day: d.day,
                            "CA Prévu": Math.round(baseRev * factor * 1.05),
                            "CA Réel": Math.round(baseRev * factor)
                        };
                    });

                    const baseRet = stats?.loyaltyRate || 35;
                    const dynamicRetentionData = MOCK_RETENTION_DATA.map((d, i) => ({
                        month: d.month,
                        rate: Math.min(100, Math.round(baseRet * (0.8 + i * 0.05)))
                    }));

                    const dynamicDemandForecast = MOCK_DEMAND_FORECAST.map(d => ({
                        hour: d.hour,
                        Déjeuner: Math.round(d.Déjeuner * ((stats?.totalVisits || 150) / 150)),
                        Dîner: Math.round(d.Dîner * ((stats?.totalVisits || 150) / 150))
                    }));
                    ...
                    return (
                        <>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                ...
                                >
                                    <AreaChart data={dynamicRevenueForecast}>
                                ...
                                >
                                    <AreaChart data={dynamicRetentionData}>
                                        <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e5e7eb" />
                                ...
                                >
                                    <BarChart data={dynamicDemandForecast}>
                                        <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e5e7eb" />
