...
const AuthScreen: React.FC = () => {
    ...
    const DEMO_CREDENTIALS = [
        { email: 'client@loycal.com', role: 'Client', icon: '👤', color: 'primary' as const },
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (USE_FIREBASE) {
                await firestoreService.registerClient(email, password, first || name, last || '');
            } else {
    ...
    const fillDemo = async (emailStr: string) => {
        setEmail(emailStr);
        if (USE_FIREBASE) {
            // Avec Firebase, le mot de passe est celui défini dans la configuration de démo
            setPassword('DEMO_PASSWORD_HERE');
        } else {
                ...
                <div className="bg-white/80 backdrop-blur-xl py-10 px-6 shadow-2xl rounded-3xl border border-gray-200">
                    ...
                    <div className="flex bg-gray-100 p-1.5 rounded-2xl mb-8">
                        <button
                            ...
                            className={`flex-1 py-3 px-4 text-sm font-semibold rounded-xl transition-all ${view === 'login'
                        ...
                        <button
                            ...
                            className={`flex-1 py-3 px-4 text-sm font-semibold rounded-xl transition-all ${view === 'register'
                                ? 'bg-white text-gray-900 shadow-md'
                                : 'text-gray-600 hover:text-gray-900'
                                }`}
                        >
